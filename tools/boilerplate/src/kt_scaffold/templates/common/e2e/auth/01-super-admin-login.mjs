...
await withBrowser(
  ...
  async ({ page, assert }) => {
    ...
    await page.getByRole("heading", { name: @@PRODUCT_NAME_JSON@@ }).waitFor();
    const session = await page.evaluate(() =>
      JSON.parse(localStorage.getItem("@@PRODUCT_SLUG@@.user") || "null"),
    );
    ...
    const missingTenant = await page.evaluate(async (apiPrefix) => {
      const token = localStorage.getItem("@@PRODUCT_SLUG@@.access_token");
    ...
    const me = await page.evaluate(
      async ({ apiPrefix, tenantHeader }) => {
        ...
        const user = JSON.parse(
          localStorage.getItem("@@PRODUCT_SLUG@@.user") || "null",
        );
        const response = await fetch(apiPrefix + "/auth/me", {
          headers: {
      ...
      { apiPrefix: "@@API_PREFIX@@", tenantHeader: "@@TENANT_HEADER@@" },
    );
    ...
    const cleared = await page.evaluate(() => ({
      token: localStorage.getItem("@@PRODUCT_SLUG@@.access_token"),
      user: localStorage.getItem("@@PRODUCT_SLUG@@.user"),
    }));
