...
backend_static() {
  # Docker Desktop presents Windows bind-mounted modules as executable. Check an identical
  # disposable copy with ordinary module modes; never chmod the user's checkout or ignore EXE002.
  backend_shell 'set -eu
    lint_root=$(mktemp -d /tmp/voiceup-static.XXXXXXXX)
    trap '\''rm -rf "$lint_root"'\'' EXIT
    cp -R app tests pyproject.toml "$lint_root/"
    find "$lint_root/app" "$lint_root/tests" -type f -name "*.py" -exec chmod 0644 {} +
    cd "$lint_root"
...
create_test_database() {
  ...
  profile_code="python"
  database_nonce="$(python3 -c 'import secrets; print(secrets.token_hex(6))')" || return $?
  test_database="${short_package}_${profile_code}_${database_nonce}_test"
  if ! printf '%s\n' "$test_database" | grep -Eq '^[a-z0-9_]+_test$'; then
    echo "refusing unsafe disposable database name" >&2
    return 2
  fi
  # run_step invokes this function inside an if, where POSIX shells disable errexit.
  # Propagate creation failure explicitly and never drop a pre-existing database.
  postgres_sql "CREATE DATABASE \"$test_database\";" || return $?
...
browser_readiness() {
  base_url="${APP_E2E_BASE:-http://127.0.0.1:8081}"
