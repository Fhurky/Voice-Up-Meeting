...
stack_backend() {
  if [ -n "$database_override" ]; then
    ...
    # Local application/worker connections have DML only. Schema commands use the separate
    # short-lived migration process, whose credentials are absent from the web container.
    "$root/scripts/stack.sh" run --rm --no-deps --pull never -T migrate "$@"
  fi
}
