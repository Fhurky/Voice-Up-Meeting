...
def decode_audio(payload: bytes, settings: Settings) -> Audio:
    if not payload:
    ...
    wav = len(payload) >= 12 and payload[:4] in (b"RIFF", b"RF64") and payload[8:12] == b"WAVE"
    ...
    try:
        with sf.SoundFile(BytesIO(payload)) as source:
            ...
            if rate < 8000 or rate > 192000 or not 1 <= channels <= 8 or frames <= 0:
                raise InferenceError("invalid_audio", "Unsupported audio dimensions", 400)
            # 24 million decoded float samples bound temporary PCM allocation to
            # about 96 MB even when a small compressed file advertises large PCM.
            if frames / rate > settings.max_duration_seconds or frames * channels > 24_000_000:
                raise InferenceError(
        ...
        raise InferenceError("invalid_audio", "Audio file cannot be decoded", 400) from None
    if len(samples) != frames or not np.isfinite(samples).all() or np.max(np.abs(samples)) > 1:
