...
`/api/voiceup/v1/auth/login`. Send the returned bearer token and the
`x-tenant-id` header to `/api/voiceup/v1/auth/me`. The login response contains the home tenant's
public identifier.
...
`VOICEUP_DATABASE_URL` at a disposable database whose name ends in `_test`.
