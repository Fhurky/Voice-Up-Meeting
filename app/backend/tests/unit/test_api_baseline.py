...
def test_export_declares_tenant_for_every_bearer_protected_operation(tmp_path: Path) -> None:
    contract = export_openapi(tmp_path / "contract.json")
    protected = []
    for path, methods in contract["paths"].items():
        for operation in methods.values():
            if isinstance(operation, dict) and any(
                "HTTPBearer" in requirement for requirement in operation.get("security", [])
            ):
                protected.append(path)
                assert all(
                    "tenant" in requirement
                    for requirement in operation["security"]
                    if "HTTPBearer" in requirement
                )
    assert f"{get_settings().api_prefix}/speaker-jobs" in protected
...
async def test_me_requires_tenant_header_before_authentication(
    client: AsyncClient,
) -> None:
    response = await client.get(f"{get_settings().api_prefix}/auth/me")
    assert response.status_code == 400
    assert response.json()["detail"]["code"] == "tenant_header_required"


async def test_me_authentication_error_has_bearer_challenge(
    client: AsyncClient,
) -> None:
