...
## Decision 6 — archive preparation compatibility

- [x] T28 Preserve archive path validation when `ntpath.isreserved` is unavailable in the declared root Python range; reject reserved device names in every path component without changing bounded extraction or source data. Evidence: six absent-API tests failed before the correction; the 40-case preparer suite passed on both Python 3.12 and 3.13. See `docs/evidence/2026-09-11-meeting-quality/archive-compatibility-report.md`; no dependency/backend runtime change or new model-quality claim.
